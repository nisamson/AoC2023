...
public class BitMatrix : ICloneable {
    ...
    public BitMatrix(int rows, int columns, bool defaultValue = false) {
        ...
        for (var i = 0; i < rows; i++) data[i] = new BitArray(columns, defaultValue);
    }
    ...
    public static BitMatrix FromArray<TNumeric>(TNumeric[,] arr) where TNumeric : INumber<TNumeric> {
        ...
        for (var i = 0; i < rows; i++) {
            for (var j = 0; j < columns; j++) result[i, j] = arr[i, j] != TNumeric.Zero;
        }
    ...
    public BitArray GetColumn(int column) {
        ...
        for (var i = 0; i < RowCount; i++) result[i] = data[i][column];
    ...
    public void SetColumn(int column, BitArray bits) {
        ArgumentOutOfRangeException.ThrowIfNotEqual(bits.Count, RowCount);
        for (var i = 0; i < RowCount; i++) data[i][column] = bits[i];
    }
    ...
    public void SetRow(int row, bool value) {
        if (!RowValid(row)) throw new ArgumentOutOfRangeException(nameof(row));

        data[row].SetAll(value);
    }

    public void SetColumn(int column, bool value) {
        if (!ColumnValid(column)) throw new ArgumentOutOfRangeException(nameof(column));

        for (var i = 0; i < RowCount; i++) data[i][column] = value;
    }

    public void SetAll(bool value) {
        foreach (var row in data) row.SetAll(value);
    }

    public BitMatrix And(BitMatrix other) {
        ArgumentOutOfRangeException.ThrowIfNotEqual(other.Size, Size);
        for (var i = 0; i < RowCount; i++) data[i].And(other.data[i]);
    ...
    public BitMatrix Or(BitMatrix other) {
        ArgumentOutOfRangeException.ThrowIfNotEqual(other.Size, Size);
        for (var i = 0; i < RowCount; i++) data[i].Or(other.data[i]);
    ...
    public BitMatrix Xor(BitMatrix other) {
        ArgumentOutOfRangeException.ThrowIfNotEqual(other.Size, Size);
        for (var i = 0; i < RowCount; i++) data[i].Xor(other.data[i]);

        return this;
    }

    public BitMatrix Not() {
        for (var i = 0; i < RowCount; i++) data[i].Not();
    ...
    public BitMatrix Clone() {
        ...
        for (var i = 0; i < RowCount; i++) result.data[i].Or(data[i]);
    ...
    public BitMatrix Transposed() {
        ...
        for (var i = 0; i < RowCount; i++) {
            for (var j = 0; j < ColumnCount; j++) result[j, i] = this[i, j];
        }
    ...
    public int CountSetBits() {
        var result = 0;
        foreach (var row in data) result += row.CountSetBits();

        return result;
    }

    public void Clear() {
        foreach (var row in data) row.Clear();
    }
    ...
    public void ClearColumn(int col) {
        for (var i = 0; i < RowCount; i++) data[i][col] = false;
    }

    public byte[,] ToBytes() {
        var result = new byte[RowCount, ColumnCount];
        for (var i = 0; i < RowCount; i++) {
            for (var j = 0; j < ColumnCount; j++) result[i, j] = this[i, j].ToByte();
        }
    ...
    public IEnumerable<(int row, int column, bool value)> EnumerateIndexed() {
        for (var i = 0; i < RowCount; i++) {
            for (var j = 0; j < ColumnCount; j++) yield return (i, j, this[i, j]);
        }
    }
}