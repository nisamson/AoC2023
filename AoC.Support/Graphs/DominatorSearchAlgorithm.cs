...
public abstract class DominatorSearchAlgorithm<TGraph, TVertex, TEdge>
    ...
    protected DominatorSearchAlgorithm(TGraph graph, TVertex root, Func<TVertex, TVertex, TEdge> edgeFactory,
        IEqualityComparer<TVertex>? comparer = null) {
        if (graph is BidirectionalMatrixPartialGraph<TVertex, TEdge> m) Graph = m;
        Graph = BidirectionalMatrixPartialGraph<TVertex, TEdge>.Create(graph, edgeFactory, comparer);
        Root = root;
