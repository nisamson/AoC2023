...
public record Hand : IComparable<Hand> {
    ...
    private HandKind CalculateAlternateKind() {
        ...
        if (jokerCount is 0 or 5) {
            return Kind;
        }

        if (jokerCount == 1 && Kind == HandKind.TwoPair) {
            return HandKind.FullHouse;
        }
    ...
    public Hand(IList<Card> cards) {
        if (cards.Count != 5) {
            throw new ArgumentException("Must have 5 cards", nameof(cards));
        }
    ...
    public int CompareToAlternate(Hand? other) {
        ...
        foreach (var (card, otherCard) in Cards.Select(c => c.ToAltCard()).Zip(other.Cards.Select(c => c.ToAltCard()))) {
            cmp = card.CompareTo(otherCard);
            if (cmp != 0) {
                return cmp;
            }
