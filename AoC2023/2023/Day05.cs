...
internal readonly record struct CategoryRanges {
    ...
    public long? Map(long value) {
        if (!SourceRange.Contains(value)) return null;
...
internal class SeedProblem {
    ...
    public long Map(Category src, Category dest, long value) {
        if (!Ranges.TryGetValue((src, dest), out var ranges)) return value;
    ...
    public long MapSeed(long seed) {
        var value = seed;
        foreach (var (src, dest) in CategoryOrder) value = Map(src, dest, value);

        return value;
    }
}

internal static class SeedLang {
    ...
    static SeedLang() {
        ...
        var categoryHeader = Nonterminal.Create(
            "categoryHeader",
            word.Extended().Append(categorySep).Append(word).Append(categorySep).Extend(word)
                .Append(Terminal.Literal("map:"))
        ...
        var categoryList =
            category
                .Many<KeyValuePair<(Category, Category), List<CategoryRanges>>,
                    Dictionary<(Category, Category), List<CategoryRanges>>>(true);
...
public class Day05 : Adventer {
    ...
    protected override void InternalOnLoad() {
        ...
        if (res.ErrorValue is not null) throw new Exception(res.ErrorValue.ToString());
