...
internal class Round {
    public Round(IEnumerable<Grab> grabs) {
        foreach (var grab in grabs) Cubes[grab.Kind] += grab.Count;
    }
...
public class Day02 : AdventBase {
    private readonly IDictionary<CubeKind, int> part1Max = new Dictionary<CubeKind, int> {
        { CubeKind.Red, 12 },
        { CubeKind.Green, 13 },
        { CubeKind.Blue, 14 }
    };
    ...
    protected override object InternalPart1() {
        ...
        foreach (var line in Input.Lines) {
            ...
            if (game.IsPossibleFor(part1Max)) total += game.Id;
        }
